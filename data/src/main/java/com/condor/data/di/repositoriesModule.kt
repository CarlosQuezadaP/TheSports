...
val teamRepositoriesModule = module {
    single<IDataSourceRemoteTeam>(named("remote_team")) { DataSourceRemoteTeam(get(), get()) }
    single<ILocalRepository<TeamDomain>>(named("local_team")) { DataSourceLocalTeam(get(), get()) }
}

val eventRepositoryModule = module {
    single<IDataSourceRemoteEvent>(named("remote_event")) { DataSourceRemoteEvent(get(), get()) }
...
val leagueRepositoryModule = module {
    ...
    single<ILocalRepository<LeagueDomain>> { DataSourceLocalLeague(get(), get()) }
}